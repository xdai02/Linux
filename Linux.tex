...
\lstset{
	...
	basewidth=0.5em,            % where to put the line-numbers
...
\begin{table}[H]
	...
	\setlength{\tabcolsep}{5mm}{
		\begin{tabular}{|c|l|}
			...
			\hline
			-c            & 表示按照最后更改时间排序，可以使用\lstinline|-l|显示出时间         \\
			\hline
			-g            & 对输出的文件不进行排序                                                     \\
			\hline
			-i            & 表示inode，打印出每个文件的inode号                                         \\
			\hline
			-l            & 将文件权限、所有者、文件大小等信息详细列出，可简写为\lstinline|ll| \\
			\hline
...
\begin{table}[H]
	...
	\setlength{\tabcolsep}{5mm}{
		\begin{tabular}{|c|l|}
			...
			\hline
			-F            & 表示separator STRING，使用指定符号替换输出文件名后默认的\lstinline|:| \\
			\hline
...
\begin{table}[H]
	...
	\setlength{\tabcolsep}{5mm}{
		\begin{tabular}{|c|l|}
			...
			-c            & 从顶部清屏并显示                                                          \\
			\hline
			-p            & 通过清除窗口而不是滚屏来对文件进行换页，与\lstinline|-c|选项相似 \\
			\hline
			-s            & 把连续的多个空行显示为一行                                                \\
			\hline
		\end{tabular}
...
\begin{table}[H]
	...
	\caption{\lstinline|killall|参数说明}
\end{table}
...
\begin{table}[H]
	...
	\caption{\lstinline|df|参数说明}
\end{table}

\subsection{du}

\lstinline|du|命令用于显示文件或目录所占用的磁盘空间。
